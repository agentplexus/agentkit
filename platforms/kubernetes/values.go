...
// These structs provide a reusable base for agent-based Helm charts with validation.
package kubernetes
...
func NewValidator() *Validator {
	...
	// Register custom validation for Kubernetes resource quantities
...
func (v *Validator) validateBusinessRules(values *Values) []error {
	...
	for name, agent := range values.Agents {
		if !agent.Enabled || agent.Service.Port == 0 {
			continue
		}
		if existing, ok := ports[agent.Service.Port]; ok {
			errs = append(errs, fmt.Errorf("port conflict: %s and %s both use port %d",
				existing, name, agent.Service.Port))
		}
		ports[agent.Service.Port] = name

		if agent.Service.A2APort > 0 {
			if existing, ok := ports[agent.Service.A2APort]; ok {
				errs = append(errs, fmt.Errorf("port conflict: %s and %s A2A both use port %d",
					existing, name, agent.Service.A2APort))
			}
			ports[agent.Service.A2APort] = name + " A2A"
		}
	}
